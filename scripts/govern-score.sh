...
[ -d modules/iam-identity-center ] && [ -d modules/ecs ] && [ -d modules/web ] && [ -d modules/alb ] && [ -d modules/acm ] && [ -d modules/cloudfront ] && [ -d modules/s3 ] && [ -d modules/vpc ] && SCORE=$((SCORE+1)) && echo "CP-9: Module dirs ... PASS" || echo "CP-9: Module dirs ... FAIL"
...
VERSIONS_OK=true
for mod in modules/iam-identity-center modules/ecs modules/web modules/alb modules/acm modules/cloudfront modules/s3 modules/vpc; do
...
HEADERS_OK=true
for tf in modules/iam-identity-center/*.tf modules/ecs/*.tf modules/web/*.tf modules/alb/*.tf modules/acm/*.tf modules/cloudfront/*.tf modules/s3/*.tf modules/vpc/*.tf; do

...
override_data {
  target = module.aws-sso.data.aws_identitystore_group.existing_sso_groups["AWSControlTowerAdmins"]
  values = {
...
override_data {
  target = module.aws-sso.data.aws_ssoadmin_permission_set.existing_permission_sets["AWSAdministratorAccess"]
  values = {
...
run "unit_test" {
  ...
  assert {
    condition     = length(module.aws-sso.sso_groups_ids) == 2

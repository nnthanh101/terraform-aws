...
module "cluster" {
  ...
  tags = merge(var.default_tags, var.tags, var.cluster_tags)
}
...
module "service" {
  ...
  tags = merge(var.default_tags, var.tags, each.value.tags)
}
...
check "ecs_required_tag_keys" {
  assert {
    ...
    error_message = "APRA CPS 234: default_tags must include CostCenter and DataClassification."
  }
}
